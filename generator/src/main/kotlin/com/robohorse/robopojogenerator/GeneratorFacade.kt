...
class GeneratorFacade {
    ...
    fun generate(model: GenerationModel, outputDir: File, packageName: String? = null): List<File> {
        if (!outputDir.exists()) {
            outputDir.mkdirs()
        }
        ...
        return if (model.useKotlin && model.useKotlinSingleDataClass) {
    ...
    private fun writeMultipleFiles(
        ...
        for (classItem in classItems) {
            val className = checkNotNull(classItem.className) {
                "Internal error: generator produced a ClassItem with null className"
            }
            ...
            written.add(writeFile(outputDir, "$className$ext", body))
        }
        return written
    }
    ...
    private fun writeSingleKotlinFile(
        ...
        targets.forEachIndexed { index, classItem ->
            ...
            rootClassBuilder.append(postProcessor.proceed(classItem, model))
        }
        ...
        return listOf(writeFile(outputDir, "${model.rootClassName}$FILE_KOTLIN", body))
    }

    private fun writeFile(outputDir: File, fileName: String, body: String): File {
